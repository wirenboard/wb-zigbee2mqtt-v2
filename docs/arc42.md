...
Старый пакет `wb-zigbee2mqtt` удаляется автоматически при установке через `Conflicts`/`Replaces` в `debian/control`. Дополнительно `postinst` удаляет переименованные пользователем скрипты v1 (`wb-zigbee2mqtt.js*`), которые не входят в пакет и не удаляются dpkg.
