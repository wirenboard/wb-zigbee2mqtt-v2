...
| Stale-устройство | устройство исчезло из `bridge/devices` | retain-топики очищены |
...
| Сценарий | Действие | Проверка |
|---|---|---|
| Безопасное имя | `"normal_device"`, `"living room lamp"` | `_is_safe_topic_name` → `True` |
| Wildcard `+` | `"device+wildcard"`, `"+"` | `_is_safe_topic_name` → `False` |
| Wildcard `#` | `"device#hash"`, `"#"` | `_is_safe_topic_name` → `False` |
| Разделитель `/` | `"room/lamp"` | `_is_safe_topic_name` → `False` |
| Пустое имя | `""` | `_is_safe_topic_name` → `False` |
| Устройство с wildcard | `bridge/devices` с `friendly_name: "evil+device"` | устройство не зарегистрировано |
...
| `device_id` непустой и не равен `zigbee2mqtt` |
...
├── unit/
│   ├── test_format_value.py         # ControlMeta.format_value / parse_wb_value (49 тестов)
│   └── test_expose_mapper.py        # map_exposes_to_controls (35 тестов)
└── integration/
